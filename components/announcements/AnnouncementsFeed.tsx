...
function PostCard({ post, onClick }: { post: Post; onClick: () => void }) {
  const preview =
    post.body_text.length > 150 ? post.body_text.slice(0, 150).trimEnd() + '…' : post.body_text

  return (
    <button
      onClick={onClick}
      className="w-full text-left border border-[#1a1a1a] hover:border-[#333] transition-colors group"
    >
      {/* Feature photo */}
      ...
      <div className="p-6">
        ...
        {/* Headline */}
        <h2 className="font-display text-2xl tracking-[3px] uppercase text-white mb-3 leading-tight group-hover:text-[#ccc] transition-colors">
          {post.headline}
        </h2>
        ...
        <p className="text-[#888] text-sm leading-relaxed">
          {preview}{' '}
          {post.body_text.length > 150 && (
            <span className="text-white underline underline-offset-2">Read More</span>
          )}
        </p>
      </div>
    </button>
...
export function AnnouncementsFeed({ initialPosts, initialTotal }: AnnouncementsFeedProps) {
  ...
  return (
    <>
      ...
      {loadingPage ? (
        <div className="py-16 text-center text-[#888] text-sm font-ui">Loading…</div>
      ) : (
      ...
      {totalPages > 1 && !loadingPage && (
        <div className="flex items-center justify-between mt-10 pt-8 border-t border-[#1a1a1a]">
          <span className="text-xs text-[#555] font-ui tracking-[1px]">
          ...
          <div className="flex gap-3">
            <button
              ...
              className="px-5 py-2 text-xs tracking-[2px] uppercase font-ui font-semibold border border-[#333] text-[#888] hover:text-white hover:border-white transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
            >
              Prev
            </button>
            <button
